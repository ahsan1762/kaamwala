...
const getDashboardStats = async (req, res) => {
    try {
        ...
        // Company Earnings from withdrawal fees
        const approvedWithdrawals = await PayoutRequest.aggregate([
            { $match: { status: 'approved' } },
            {
                $group: {
                    _id: null,
                    totalFees: { $sum: '$companyFee' },
        ...
        const companyEarnings = approvedWithdrawals[0]?.totalFees || 0;
        const totalApprovedWithdrawals = approvedWithdrawals[0]?.totalWithdrawals || 0;
        const totalAmountPaidOut = approvedWithdrawals[0]?.totalAmountPaid || 0;

        // Pending withdrawal requests
        const pendingWithdrawals = await PayoutRequest.countDocuments({ status: 'pending' });

        // Withdrawal fees by status for chart
        const withdrawalsByStatus = await PayoutRequest.aggregate([
            {
                $group: {
                    _id: '$status',
                    count: { $sum: 1 },
...
const getWithdrawalRequests = async (req, res) => {
    try {
        ...
        console.log('Fetching withdrawal requests with query:', query);
...
// @route   PATCH /api/admin/withdrawal-requests/:id/approve
// @access  Private (Admin)
const approveWithdrawalRequest = async (req, res) => {
    try {
        ...
        if (worker.availableBalance < totalDeduction) {
            return res.status(400).json({ 
                message: `Insufficient worker balance. Required: Rs. ${totalDeduction}, Available: Rs. ${worker.availableBalance}` 
            });
        }
        ...
        res.json({
            ...
            summary: {
                ...
                companyFee: request.companyFee,
                workerReceives: request.netAmount,
                totalDeducted: totalDeduction,
                workerNewBalance: worker.availableBalance
            }
        });
    } catch (error) {
        console.error(error);
...
// @route   PATCH /api/admin/withdrawal-requests/:id/reject
// @access  Private (Admin)
