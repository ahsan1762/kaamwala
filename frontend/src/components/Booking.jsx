...
const NewBooking = ({ user }) => {
    ...
    // Fetch workers for selected service from backend
    const fetchWorkersForService = async (serviceId) => {
        ...
        setWorkers([]);
        try {
            const res = await api.get(`/worker/search?skill=${encodeURIComponent(selectedServiceObj.name)}`);
            setWorkers(res.data || []);
        } catch (err) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const bookingData = {
                ...
                workerId: selectedWorker?.userId?._id || workerIdParam || null
            };
    ...
    const validateStep = (stepNum) => {
        switch (stepNum) {
            ...
            case 4: return formData.address !== '' && formData.phone !== '';
            default: return true;
        }
    };
    ...
    const stepLabels = ['Service', 'Worker', 'Schedule', 'Details', 'Confirm'];
        ...
        return (
            <div className="booking-success">
                <div className="success-card">
                    ...
                    <p>Your service request has been sent to <strong>{selectedWorker?.userId?.name || 'the worker'}</strong>.</p>
                    ...
                    <div className="success-actions">
                        ...
                        <button onClick={() => { setSuccess(false); setStep(1); setSelectedWorker(null); setFormData({ ...formData, service: '', date: '', time: '', address: '', notes: '' }); }} className="new-btn">
        ...
        <div className="new-booking">
                ...
                <div className="booking-step">
                    ...
                    <div className="step-actions">
                        <button
                            ...
                            onClick={() => { fetchWorkersForService(formData.service); setStep(2); }}
                        >
                            Continue
                        </button>
                    </div>
            ...
            {step === 2 && (
                <div className="booking-step">
                    ...
                    <p>Select a verified professional for <strong>{services.find(s => s.id === formData.service)?.name}</strong></p>

                    {workersLoading && (
                        <div className="workers-loading">
                            <div className="workers-spinner"></div>
                    ...
                    {!workersLoading && workers.length === 0 && (
                        <div className="no-workers-message">
                            <span className="no-workers-emoji">😔</span>
                            <h3>We are sorry, we don't have this service yet!</h3>
                            <p>We are working hard to bring more professionals to your area. Please check back soon.</p>
                            <button className="back-btn" style={{ marginTop: '16px' }} onClick={() => setStep(1)}>
                                ← Try Another Service
                            </button>
                        </div>
                    )}
                        ...
                        <>
                            <div className="workers-grid">
                                {workers.map(worker => (
                                    <div
                                        key={worker._id}
                                        className={`worker-select-card ${selectedWorker?._id === worker._id ? 'selected' : ''}`}
                                        onClick={() => setSelectedWorker(worker)}
                                    >
                                        <div className="worker-card-avatar">
                                            {worker.profilePic ? (
                                                <img src={worker.profilePic} alt={worker.userId?.name} />
                                            ) : (
                                                <div className="worker-avatar-placeholder">
                                                    {worker.userId?.name?.charAt(0)?.toUpperCase() || 'W'}
                                                </div>
                                            )}
                                            {selectedWorker?._id === worker._id && (
                                                <div className="worker-selected-badge">&#10003;</div>
                                            )}
                                        </div>
                                        <div className="worker-card-info">
                                            <h3>{worker.userId?.name || 'Worker'}</h3>
                                            <span className="worker-skill-tag">{worker.skill}</span>

                                            {/* Profile Details */}
                                            <div className="worker-profile-details">
                                                {worker.experience > 0 && (
                                                    <span className="profile-detail">📚 {worker.experience} {worker.experience === 1 ? 'year' : 'years'} exp.</span>
                                                )}
                                                {worker.completedJobs > 0 && (
                                                    <span className="profile-detail">✓ {worker.completedJobs} completed jobs</span>
                                                )}
                                                {worker.reviewsCount > 0 && (
                                                    <span className="profile-detail">💬 {worker.reviewsCount} reviews</span>
                                                )}
                                            </div>
                                            ...
                                            <div className="worker-meta">
                                                ...
                                                {worker.averageRating > 0 && <span>⭐ {Number(worker.averageRating).toFixed(1)}</span>}
                                            </div>

                                            <div className="worker-footer">
                                                <span className="worker-rate">PKR {worker.hourlyRate}/hr</span>
                                                <p className="worker-availability">&#128336; {worker.availability || '9AM - 6PM'}</p>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                    ...
                    <div className="booking-summary">
                        <div className="summary-card">
                            ...
                            <div className="summary-row">
                                ...
                                <strong>{selectedWorker?.userId?.name || 'Any available'}</strong>
                            </div>
                            <div className="summary-row">
                                <span>Date</span>
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            <textarea
                ...
                style={{ width: '100%', padding: '10px', borderRadius: '5px', border: '1px solid #ddd', minHeight: '60px' }}
            />
            {error && (
